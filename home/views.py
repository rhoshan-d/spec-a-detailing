...
def index(request):
    ...
    if request.user.is_superuser:
        ...
        user_reviews = CustomerReview.objects.filter(
            user=request.user, approved=False).order_by('-created_date')
        reviews = (approved_reviews | user_reviews).distinct().order_by(
            '-created_date')[:4]
    else:
        reviews = CustomerReview.objects.filter(
            approved=True).order_by('-created_date')[:4]
...
def delete_review(request, review_id):
    ...
    if review.user == request.user:
        if not review.approved:
            ...
            messages.error(
                request, 'You cannot delete reviews that have been approved. Please contact admin.')
    else:
...
def approve_review(request, review_id):
    ...
    if not request.user.is_superuser:
        messages.error(
            request, 'Sorry, only staff members can approve reviews.')
        return redirect(reverse('home'))
...
def edit_review(request, review_id):
    ...
    if review.approved and not request.user.is_superuser:
        messages.error(
            request, 'Sorry, you cannot edit reviews that have already been approved.')
        return redirect(reverse('home'))
