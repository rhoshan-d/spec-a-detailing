...
def book_service(request, service_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Your booking for {service.name} on {booking.booking_date} has been received!")
...
def cancel_booking(request, booking_id):
    ...
    if request.method == 'POST':
        if booking.can_cancel():
            ...
            booking.save()
            messages.success(request, f"Your booking for {booking.service.name} has been cancelled.")
        else:
            messages.error(request, "Bookings can only be cancelled at least 24 hours in advance.")
